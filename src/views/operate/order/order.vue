							...
							<div class="card-header u-flex u-flex-between">
								<div class="item u-flex">
									<!-- <el-image style="width: 30px; height: 30px; border-radius: 5px;" :src="sell_info.img"
										fit="cover" lazy /> -->
									<span class="u-m-l-10">{{ list.sell_login }}</span>
								</div>
							</div>
						</template>
		...
		<div class="u-p-20 box"> 
			...
			<el-row :gutter="10">
				...
				<el-col :span="19" class="u-flex"> 
					...
					<div class="item u-m-r-10" v-show="list.status == '0'">
						<el-popconfirm 
							title="支付订单确认" 
							@confirm="payOrderBtn({order_id: list.id})"
							confirm-button-text="确认"
							cancel-button-text="取消"
							>
							<template #reference>
								<el-button plain type="primary"  :loading="pay_loading">支付订单</el-button>	 
							</template>
						</el-popconfirm>
					</div>
					<div class="item u-m-r-10" v-show="list.status == '0'">
						<el-popconfirm 
							title="取消订单确认" 
							@confirm="cancelOrderBtn({order_id: list.id})"
							confirm-button-text="确认"
							cancel-button-text="取消"
							>
							<template #reference>
								<el-button plain type="danger"  >取消订单</el-button>	 
							</template>
						</el-popconfirm>
					</div>
					<div class="item u-m-r-10" v-show="list.status == '1'">
						<el-popconfirm 
							title="退款申请确认" 
							@confirm="refundBtn({order_id: list.id})"
							confirm-button-text="确认"
							cancel-button-text="取消"
							>
							<template #reference>
								<el-button plain type="danger"  >退款申请</el-button>	 
							</template>
						</el-popconfirm>
					</div>
